...
EOF
```
As observed, both primal and dual SVMs produce identical results, achieving a maximum accuracy of 88%, which aligns with theoretical expectations.

# Section 2: applying SVMs to a new dataset. 

As a real-world testing data for our SVMs we have selected Banknote dataset (https://archive.ics.uci.edu/dataset/267/banknote+authentication). Images of genuine and forged banknotes were digitized using an industrial camera (400x400 pixels, 660 dpi). Theree are 4 features whcih were extracted using wavelet transformation and 1 target:
...
param svi default 0;
for {i in 1..m} {
    if lambda[i] > 1e-10 and lambda[i] < nu then {
        let svi := i;
        break;
    }
}
...
EOF
```
As we can see both primal and dual SVMs efficiently produce prediction of the money class with accuracy of (0.97-0.98), which is an outstanding result. The slight difference (by less than 0.2) in $\gamma$ between primal and dual can be explained by the numerical precision differences in calculation the methods in ampl???.
